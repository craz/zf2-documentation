...
(шлюз к  данным таблицы) может стать проблемой в больших системах. Существует также искушение поместить код
доступа к базе данных в методах контроллера, используя представляемые
``Zend\Db\TableGateway\AbstractTableGateway`` методы. *Не делайте этого *!
